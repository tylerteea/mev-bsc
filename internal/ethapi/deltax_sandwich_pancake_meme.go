...
func (s *BundleAPI) SandwichBestProfitPanMeme(ctx context.Context, sbp SbpPanMemeArgsNew) map[string]interface{} {
...
func workerPanMemeNew(
	...
	defer func() {
		if r := recover(); r != nil {
			...
			log.Info("recover...call_workerPanMeme", "reqAndIndex", reqAndIndex, "err", r, "stack", dss)
		}
	}()
...
func executePanMemeNew(
	...
	if isFront {
		...
		data := encodeParamsPanMemeBackNew(sbp.Token, amountIn, sbp.Balance, builderAddress, briberyWei)
		bytes := hexutil.Bytes(data)
		callArgs = &TransactionArgs{
			From:  &sbp.Eoa,
			To:    &sbp.Contract,
...
func encodeParamsPanMemeFrontNew(
	...
	if methodId == "87f27655" || methodId == "e671499b" {
...
func encodeParamsPanMemeBackNew(
