...
// SandwichBestProfitPanMemeEoa profit calculate
func (s *BundleAPI) SandwichBestProfitPanMemeEoa(ctx context.Context, sbp SbpPanMemeEoaArgs) map[string]interface{} {
	...
	bestInFunc := func(x []float64) float64 {
		...
		workerResults := workerPanMemeEoa(ctx, head, victimTransaction, sbp, s, reqId, stateDB, amountInInt)
	...
	sdb := stateDBNew.Copy()
	workerResults := workerPanMemeEoa(ctx, head, victimTransaction, sbp, s, reqAndIndex, sdb, quoteAmountIn)
...
func workerPanMemeEoa(
	ctx context.Context,
	head *types.Header,
	victimTransaction *types.Transaction,
	sbp SbpPanMemeEoaArgs,
	s *BundleAPI,
	reqAndIndex string,
	...
	defer func() {
		if r := recover(); r != nil {
			...
			log.Info("recover...call_workerPanMemeEoa", "reqAndIndex", reqAndIndex, "err", r, "stack", dss)
		}
	}()
	...
	// 抢跑----------------------------------------------------------------------------------------
	fErr := executePanMemeEoa(ctx, reqAndIndex, true, sbp, amountIn, statedb, s, head)
	...
	bErr := executePanMemeEoa(ctx, reqAndIndex, false, sbp, backAmountIn, statedb, s, head)
